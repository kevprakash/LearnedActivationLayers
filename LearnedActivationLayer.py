...
class TuningActivationLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        self._xs = self.add_weight(name='horizontalScale', shape=(1,), initializer=keras.initializers.RandomNormal(mean=1, stddev=0.005), trainable=True)
        self._xb = self.add_weight(name='horizontalBias',  shape=(1,), initializer=keras.initializers.RandomNormal(mean=0, stddev=0.005), trainable=True)
        self._ys = self.add_weight(name='verticalScale',   shape=(1,), initializer=keras.initializers.RandomNormal(mean=1, stddev=0.005), trainable=True)
        self._yb = self.add_weight(name='verticalBias',    shape=(1,), initializer=keras.initializers.RandomNormal(mean=0, stddev=0.005), trainable=True)
...
class LearnedFourierActivationLayer(Layer):
    ...
    def call(self, x, mask=None):
        ...
        for i in range(0, self._N):
            s = s + (self._p[i] * K.cos(2 * math.pi * i * x / self._N)) + (self._q[i] * K.sin(2 * math.pi * i * x / self._N))
        return s
